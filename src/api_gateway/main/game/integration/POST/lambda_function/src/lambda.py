...
def make_app_sync_request_mutation(
    event_body: GameMutation, event_headers: HTTPHeaders
) -> HTTPResponse:
...
def handler(event: Event, context: Any) -> Response:
    body: Optional[Game] = None
    headers: HTTPHeaders = {"Content-Type": "application/json"}
    status_code: int = HTTPStatus.OK.value
    try:
        event_body: GameMutation = json.loads(event.get("body"))
        ...
        print(response_data)
        body = response_data.get("data", {}).get("create_game")
