...
def make_app_sync_request_mutation(
    event_body: PiiMutation, event_headers: HTTPHeaders
) -> HTTPResponse:
...
def handler(event: Event, context: Any) -> Response:
    body: Optional[Pii] = None
    headers: HTTPHeaders = {"Content-Type": "application/json"}
    status_code: int = HTTPStatus.OK.value
    try:
        event_body: PiiMutation = json.loads(event.get("body"))
        ...
        print(response_data)
        body = response_data.get("data", {}).get("create_pii")
