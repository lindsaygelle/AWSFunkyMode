...
class SyntaxTokenMutation(TypedDict):
    begin_offset: int
    end_offset: int
    id: UUID
    order: int
...
def make_app_sync_request_mutation(
    event_body: SyntaxTokenMutation, event_headers: HTTPHeaders
) -> HTTPResponse:
...
def handler(event: Event, context: Any) -> Response:
    body: Optional[SyntaxToken] = None
    headers: HTTPHeaders = {"Content-Type": "application/json"}
    status_code: int = HTTPStatus.OK.value
    try:
        event_body: SyntaxTokenMutation = json.loads(event.get("body"))
        ...
        print(response_data)
        body = response_data.get("data", {}).get("create_syntax_token")
