...
class SyntaxTokenConnection(TypedDict):
    items: Optional[List[SyntaxToken]]
...
def handler(event: Event, context: Any) -> Response:
    body: Optional[SyntaxTokenConnection] = None
    ...
    try:
        ...
        print(response_data)
        body = (response_data.get("data") or {}).get("get_syntax_token_connection")
