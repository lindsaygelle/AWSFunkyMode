...
mutation Mutation(
    ...
    create_quote_subscription_rule(input: {
        address: $address,
        category: $category,
        quote_subscription_id: $quote_subscription_id,
        schedule: $schedule
    }) {
        address
        category
...
class QuoteSubscriptionRuleMutation(TypedDict):
    address: str
    category: str
    quote_subscription_id: str
    schedule: str
...
def make_app_sync_request_mutation(
    event_body: QuoteSubscriptionRuleMutation, event_headers: HTTPHeaders
) -> HTTPResponse:
...
def handler(event: Event, context: Any) -> Response:
    body: Optional[QuoteSubscriptionRule] = None
    headers: HTTPHeaders = {"Content-Type": "application/json"}
    status_code: int = HTTPStatus.OK.value
    try:
        event_body: QuoteSubscriptionRuleMutation = json.loads(event.get("body"))
        ...
        print(response_data)
        body = response_data.get("data", {}).get("create_quote_subscription_rule")
