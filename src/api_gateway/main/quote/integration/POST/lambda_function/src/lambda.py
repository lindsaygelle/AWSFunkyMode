...
def make_app_sync_request_mutation(
    event_body: QuoteMutation, event_headers: HTTPHeaders
) -> HTTPResponse:
...
def handler(event: Event, context: Any) -> Response:
    ...
    try:
        event_body: QuoteMutation = json.loads(event.get("body"))
        ...
        print(response_data)
        body = response_data.get("data", {}).get("create_quote")
