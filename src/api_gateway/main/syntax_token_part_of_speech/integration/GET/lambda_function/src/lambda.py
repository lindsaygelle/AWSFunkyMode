...
query Query($limit: Int, $next_token: String) {
    get_syntax_token_part_of_speech_connection(limit: $limit, next_token: $next_token) {
        items {
            created_date
            score
            id
...
def handler(event: Event, context: Any) -> Response:
    logging.info(event)
    body: Optional[SyntaxTokenPartOfSpeechConnection] = None
    ...
    try:
        ...
        if isinstance(body, dict):
            body = body.get("get_syntax_token_part_of_speech_connection")
